...
const CollaborationHub = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if user is an photographer
        if (user && user.role !== 'photographer') {
          setError('This feature is only available to photographers');
          setLoading(false);
          return;
        }
  ...
  // If user is not an photographer, show restricted access message
  if (user && user.role !== 'photographer') {
    return (
      <div className="flex flex-col min-h-screen bg-cover bg-center text-white" style={{ backgroundImage: "url('piano-background.jpg')" }}>
        <Navbar />
        <div className="container mx-auto px-4 py-8">
          <h1 className="text-3xl font-bold text-white mb-6">Photographer Collaboration Hub</h1>
          <div className="bg-red-500 text-white p-4 rounded-md mb-6">
            This feature is only available to photographers. Please upgrade your account to access the collaboration hub.
          </div>
        </div>
      </div>
    );
            ...
            <form onSubmit={handleCreatePost} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="E.g., Looking for a photographer for an outdoor shoot"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={newPost.genre}
                    onChange={handleInputChange}
                    placeholder="E.g., Landscape, Fashion, Portrait, Documentary"
                    className="w-full p-3 border rounded-md text-base bg-white/10 text-white placeholder-white/60 border-white/30"
                  />
                </div>
                <div>
