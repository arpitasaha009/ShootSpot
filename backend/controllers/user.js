...
export const getUserReleases = async (req, res) => {
  try {
    ...
    if (user.role !== 'photographer') {
      return res.status(403).json({ message: 'Only photographer can have releases' });
    }
    
    // Find releases for the user
    const releases = await Release.find({ photographer: userId }).sort({ createdAt: -1 });
...
export const addRelease = async (req, res) => {
  try {
    ...
    if (user.role !== 'photographer') {
      return res.status(403).json({ message: 'Only photographer can add releases' });
    }
...
export const deleteRelease = async (req, res) => {
  try {
    ...
    if (release.photographer.toString() !== userId) {
